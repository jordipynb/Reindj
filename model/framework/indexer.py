...
class Indexer(ABC):
    ...
    @abstractmethod
    def __call__(self, docs_bodies:list[str]) -> tuple[np.ndarray,np.ndarray,list[str]]:pass
...
class VectorIndexer(Indexer):
    ...
    def __get_weight__(self,N_doc:int,n_terms:int,dict_terms:dict[str,dict[int,int]],max_frequency:list[int]) -> tuple[np.ndarray,np.ndarray,list[str]]:
...
class Latent_Semantic_Indexer(Indexer):
    ...
    def __get_SVD__(self,tf:dict[str,np.ndarray],tf_2:dict[str,int],d:int,k:int) -> tuple[np.ndarray,np.ndarray,np.ndarray]:
        t=len(tf.keys())
