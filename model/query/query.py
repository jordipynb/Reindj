...
class Query:
    ...
    def __get_queries__(self) -> list[Qrydb]:
        if not self.__path__.exists() or not self.__path__.is_file(): raise Exception("Queries path does not exist")
        queries = ""
        with open(self.__path__, "r", encoding='utf8') as queriespath:
            queries = queriespath.read()
